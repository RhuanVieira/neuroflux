...
const getCookie = (req: Request, name: string) => req.headers.cookie?.split(";").map(value => value.trim()).find(value => value.startsWith(`${name}=`))?.slice(name.length + 1);

export function startGoogleLogin(_req: Request, res: Response) {
  if (!google) return res.status(503).json({ message: "O login Google ainda não foi configurado no servidor." });
...
export async function completeGoogleLogin(req: Request, res: Response) {
  if (!google || !clientId) return res.status(503).send("O login Google ainda não foi configurado no servidor.");
  ...
  if (!expectedState || !state || state.length !== expectedState.length || !crypto.timingSafeEqual(Buffer.from(state), Buffer.from(expectedState))) return res.status(400).send("A validação de segurança do login expirou. Tente novamente.");
  try {
    const code = typeof req.query.code === "string" ? req.query.code : "";
    const { tokens } = await google.getToken(code);
    if (!tokens.id_token) throw new Error("O Google não retornou um ID token.");
    const ticket = await google.verifyIdToken({ idToken: tokens.id_token, audience: clientId });
    const profile = ticket.getPayload();
    if (!profile?.sub || !profile.email || !profile.email_verified) throw new Error("A conta Google não possui e-mail verificado.");
    const email = profile.email.toLowerCase();
    const existing = await prisma.user.findFirst({ where: { OR: [{ googleId: profile.sub }, { email }] } });
    if (existing?.role === Role.MASTER) return res.status(403).send("Contas MASTER devem usar o login normal.");
    if (existing?.googleId && existing.googleId !== profile.sub) return res.status(409).send("Este e-mail já está vinculado a outra conta Google.");
  ...
  } catch (error) { return res.status(401).send(error instanceof Error ? `Não foi possível concluir o login Google: ${error.message}` : "Não foi possível concluir o login Google."); }
}
