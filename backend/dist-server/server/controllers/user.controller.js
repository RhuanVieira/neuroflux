...
async function paginatedUsers(role, req) {
...
export async function listAdmins(req, res, next) {
    try {
        return res.json(await paginatedUsers(Role.ADMIN, req));
    }
    catch (error) {
        return next(error);
    }
...
export async function deleteAdmin(req, res, next) {
    try {
        const id = z.coerce.number().int().positive().parse(req.params.id);
        if (id === req.user.id)
            return res.status(400).json({ message: "Você não pode remover a própria conta." });
        const target = await prisma.user.findUnique({ where: { id } });
        if (!target || target.role !== Role.ADMIN)
            return res.status(404).json({ message: "Administrador não encontrado." });
...
export async function listStudents(req, res, next) {
    try {
        return res.json(await paginatedUsers(Role.STUDENT, req));
    }
    catch (error) {
        return next(error);
    }
